    ...
    if normalize:
        if norm == "l2":
            magnitude = torch.sqrt(
                torch.sum(torch.square(x_zm), dim=axis, keepdim=True)
            )
...
def print_options(parser, opt):
    ...
    taken from: https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix/blob/master/options/base_options.py  # noqa: E501
    """
    message = ""
    message += "----------------- Options ---------------\n"
...
def save_recon(under, recon, reference, index, save_dir, error_scale=1, do_save=True):
    ...
    for i in range(batch_size):
        ...
        img_to_save = 255 * np.concatenate(
            (under_mag, recon_mag, reference_mag, diff), axis=1
        )
        if do_save:
