...
def removeFEOversampling(src):
    ...
    if nPE != nFE:
        return np.take(
            src, np.arange(int(nFE * 0.25) + 1, int(nFE * 0.75) + 1), axis=-2
        )
    else:
        return src
...
def removePEOversampling(src):
    ...
    if PE_OS_crop == 0:
        ...
        return np.take(
            src, np.arange(int(PE_OS_crop) + 1, nPE - int(PE_OS_crop) + 1), axis=-1
        )


def fft2c(img):
    """Centered fft2"""
    return np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(img))) / np.sqrt(
...
def ifft2c(img):
    """Centered ifft2"""
    return np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(img))) * np.sqrt(
        img.shape[-2] * img.shape[-1]
    )
