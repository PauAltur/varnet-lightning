...
class RBFActivation(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs["init_type"] == "linear":
            ...
            alpha = 100
            w_0 = (
                kwargs["init_scale"] * np.sqrt(alpha) * x_0 / (1 + 0.5 * alpha * x_0**2)
            )
        else:
...
class VnMriReconCell(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        conv_kernel = np.random.randn(
            ...
            options["kernel_size"] ** 2 * 2 * options["features_in"]
        )
        ...
        self.lamb = torch.nn.Parameter(
            torch.tensor(options["lamb_init"], dtype=torch.float32)
        )
    ...
    def forward(self, inputs):
        ...
        u_t_real = F.pad(
            u_t_real, [pad, pad, pad, pad], mode="reflect"
        )  # to do: implement symmetric padding
        ...
        u_k_T_real = F.conv_transpose2d(
            f_u_k, self.conv_kernel[:, :, :, :, 0], stride=1, padding=5
        )
        u_k_T_imag = F.conv_transpose2d(
            f_u_k, self.conv_kernel[:, :, :, :, 1], stride=1, padding=5
        )
