...
import projectB.utils.mri as mri_utils
...
DEFAULT_OPTS = {
    ...
    "eval_slices": [x for x in range(10, 30)],
    "mode": "train",
    "load_target": True,
...
class KneeDataset(Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        if options["mode"] == "train":
            ...
            slice_no = [
                x for x in range(options["start_slice"], options["end_slice"] + 1)
            ]
    ...
    def __getitem__(self, idx):
        ...
        f = np.ascontiguousarray(np.transpose(raw_data["rawdata"], (2, 0, 1))).astype(
            np.complex64
        )

        coil_sens_data = loadmat(coil_sens)
        c = np.ascontiguousarray(
            np.transpose(coil_sens_data["sensitivities"], (2, 0, 1))
        ).astype(np.complex64)
        ...
        if self.options["sampling_pattern"] == "cartesian_with_os":
            ...
            raise ValueError(
                "sampling_pattern has to be in [cartesian_with_os, cartesian]"
            )
